      ...
      <div className="max-w-7xl mx-auto px-2 md:px-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {allSkills.map((skill, index) => (
            <div
              key={index}
              className="flex flex-col bg-white rounded-4px shadow-md p-6 border border-transparent hover:border-font/10 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group"
            >
              {/* Thẻ Tag tên Skill: Bo tròn trịa, viền mảnh chuẩn bài như tag "UI/UX Designer" bên About */}
              <div className="flex mb-4">
                <span className=" text-primary font-akshar font-bold uppercase tracking-wider text-xs md:text-sm rounded-full px-4 py-1.5 inline-block group-hover:bg-font group-hover:text-background transition-colors duration-300">
                  {skill.name}
                </span>
              </div>
