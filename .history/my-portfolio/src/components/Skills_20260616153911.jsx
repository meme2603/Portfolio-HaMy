          ...
          {allSkills.map((skill, index) => (
            ...
            >
              <div className="flex mb-4">
                <span className=" text-font font-akshar font-semibold uppercase tracking-wider text-xs md:text-sm lg:text-base rounded-full px-4 py-1 inline-block group-hover:bg-primary group-hover:text-background transition-colors duration-300">
                  {skill.name}
                </span>
              </div>

              <p className="whitespace-pre-linetext-xs md:text-sm leading-relaxed text-font/70 font-light tracking-wide pt-1 flex-grow">
                {skill.desc}
              </p>
            </div>
          ))}
