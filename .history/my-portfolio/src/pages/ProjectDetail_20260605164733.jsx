    ...
    <section className="w-full bg-background text-font py-10">
      <div className="w-full mx-auto">
        ...
        <div className="w-full pt-24 pb-4 px-20">
          ...
          >
            {allProjects
              .filter((item) => item.id !== id) // Lọc bỏ cái dự án đang xem
              .map((item) => (
                <Link 
                  key={item.id} 
                  to={`/project/${item.id}`}
                  {/* 👇 THẦN CHÚ: Trên PC tự động chia đều vừa khít 3 card/màn hình, khum bị bé, khum bị cắt chiều cao */}
                  className="min-w-[280px] sm:min-w-[calc((100%-24px)/2)] md:min-w-[calc((100%-48px)/3)] flex-shrink-0 snap-start group block"
                >
                  {/* Khung ảnh tự co giãn theo chiều cao gốc của ảnh bà đã import */}
                  <div className="w-full h-auto overflow-hidden bg-[#1A120B]/5 rounded-sm shadow-sm border border-[#1A120B]/5">
                    <img 
                      src={item.img} 
                      alt={item.title} 
