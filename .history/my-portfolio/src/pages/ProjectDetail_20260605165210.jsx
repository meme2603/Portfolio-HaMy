    ...
    <section className="w-full bg-background text-font py-10">
      <div className="w-full mx-auto">
        ...
        <div className="w-full pt-24 pb-4 px-20">
          ...
          <div
            ref={scrollRef}
            className="flex gap-6 overflow-x-auto scroll-smooth mb-24 snap-x snap-mandatory pb-4 no-scrollbar"
            ...
            {allProjects
              ...
              .map((item) => (
                <Link
                  key={item.id}
                  to={`/project/${item.id}`}
                  {/* 🎯 SỬA ĐÚNG DÒNG NÀY: Dùng công thức calc để PC hiện vừa khít 3 card bự, thêm flex-shrink-0 để khum bị co hình */}
                  className="min-w-[280px] sm:min-w-[calc((100%-24px)/2)] md:min-w-[calc((100%-48px)/3)] flex-shrink-0 snap-start group block"
                >
                  {/* Bên dưới giữ nguyên 100% y chang code cũ của bà, không thay đổi một chữ nào */}
