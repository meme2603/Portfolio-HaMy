...
function ProjectDetail() {
  ...
  return (
    // 🎯 THẦN CHÚ: Thêm key={id} ở đây để auto cuộn trang khi bấm chuyển dự án nha bà!
    <div key={id} className="w-full min-h-screen bg-[#FAF7F2] text-[#1A120B]">
      
      {/* Khối hiển thị layout động */}
      {isWebsite ? renderWebsiteLayout() : renderAppLayout()}

      {/* ─── 共通 BLOCK 6: KHỐI DÙNG CHUNG ĐÃ ĐƯỢC RESPONSIVE ─── */}
      <div className="w-full pt-16 md:pt-24 pb-6 px-6 sm:px-12 md:px-20">
        
        {/* Tiêu đề & Cụm nút chuyển slide */}
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-6 mb-8 md:mb-12">
          ...
          {/* Cụm nút bấm: Ẩn bớt trên Mobile (vì dùng tay vuốt sướng hơn), hiện từ màn hình SM trở lên */}
          <div className="hidden sm:flex gap-4 text-[#1A120B] pr-2 select-none">
            <button
              onClick={() => {
                if (scrollRef.current) scrollRef.current.scrollLeft -= scrollRef.current.offsetWidth / 3;
              }}
        ...
        {/* Thanh trượt Card: Tự chia 1 card (Mobile), 2 card (Tablet) và 3 card (PC) chuẩn chỉ */}
        <div
          ref={scrollRef}
          className="flex gap-4 md:gap-6 overflow-x-auto scroll-smooth mb-16 md:mb-20 snap-x snap-mandatory pb-4 no-scrollbar"
          ...
          {allProjects
            ...
            .map((item) => (
              <Link
                key={item.id}
                to={`/project/${item.id}`}
                {/* 🎯 RESPONSIVE CARD BÙA CHÚ: Tự đo layout theo % để khum bị vỡ tỉ lệ */}
                className="min-w-[calc(100%-16px)] sm:min-w-[calc((100%-24px)/2)] md:min-w-[calc((100%-48px)/3)] flex-shrink-0 snap-start group block"
              >
        ...
        <div className="w-full border-t border-[#1A120B]/10 pt-6 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs md:text-sm font-light text-[#1A120B]/80 tracking-wide">
          <div className="flex items-center gap-2 font-sans flex-wrap justify-center sm:justify-start">
          ...
          <div className="font-sans text-[#1A120B]/70 text-center sm:text-right">
