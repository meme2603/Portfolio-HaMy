...
import { useRef, useEffect } from "react"; // 🎯 FIX 1: Đã thêm useEffect vào đây để khum lỗi biến
...
function ProjectDetail() {
  ...
  // Gộp 2 mảng dữ liệu thực tế từ file data của bà
  const allProjects = [...websiteProjects, ...appProjects];
  const project = allProjects.find((p) => p.id === id);
  const isWebsite = websiteProjects.some((p) => p.id === id);

  // 🎯 FIX 2: Ép trình duyệt nhảy lên đầu trang ngay lập tức khi đổi dự án để mượt mà 100%
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [id]);

  // Nếu khum tìm thấy dự án nào trùng ID thì chặn lại khum cho vỡ trang
  if (!project) {
    return (
  ...
  // ─── 🎯 FIX 3: LỆNH RETURN GỐC DUY NHẤT (Đã bọc key={id} và gộp BLOCK 6 sạch sẽ) ───
  return (
    <div key={id} className="w-full min-h-screen bg-[#FAF7F2] text-[#1A120B]">
      {/* Gọi layout động tùy biến dự án */}
      {isWebsite ? renderWebsiteLayout() : renderAppLayout()}

      {/* BLOCK 6: "他にも見る" DÙNG CHUNG KHÔNG ĐỔI */}
      <div className="w-full pt-24 pb-4 px-20">
              ...
              onClick={() => {
                if (scrollRef.current)
                  scrollRef.current.scrollLeft -=
                    scrollRef.current.offsetWidth / 3;
              }}
        ...
        {/* 🎯 FIX 4: Ép hiển thị vừa vặn 3 card bự trên PC, ẩn 2 card để scroll sang */}
        <div
          ref={scrollRef}
          ...
          {allProjects
            ...
            .map((item) => (
              <Link
                key={item.id}
                to={`/project/${item.id}`}
                className="min-w-[280px] sm:min-w-[calc((100%-24px)/2)] md:min-w-[calc((100%-48px)/3)] flex-shrink-0 snap-start group block"
              >
