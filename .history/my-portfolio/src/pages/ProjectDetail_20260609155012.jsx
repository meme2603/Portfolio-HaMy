  ...
  const renderPortfolioLayout = () => {
    return (
      <section className="w-full bg-[#FAF7F2] text-[#1A120B] py-10">
        <div className="w-full mx-auto">
          {/* 1. Khung Hero độc quyền cho Portfolio - Thêm hẳn bg/padding bảo hiểm cho ảnh */}
          {project.mockupHero && (
            <div className="w-full max-w-7xl mx-auto mt-24 mb-10 flex justify-center bg-zinc-200/40 p-4 rounded-sm">
              <img
                src={project.mockupHero}
                alt="Portfolio Premium Hero"
                className="w-full h-fit object-contain shadow-md block"
              />
            </div>
          )}

          {/* 2. Khối Intro giới thiệu Portfolio */}
          <div className="w-full max-w-7xl mx-auto mb-20 px-6 md:px-0">
            {/* Bà muốn thiết kế chữ nghĩa, thông tin của Portfolio thế nào thì cứ thả vô tư vào đây */}
            <h1 className="text-4xl font-bold uppercase">{project.title}</h1>
            <p className="pt-4 text-sm font-light leading-relaxed max-w-2xl">
              {project.desc}
            </p>
          </div>

          {/* 3. Khối hiển thị Mockup PC làm lại mới toanh (Căn giữa full không viền dọc) */}
          {project.mockupPC && (
            <div className="w-full max-w-5xl mx-auto mb-24 px-6 md:px-0 flex justify-center">
              <img
                src={project.mockupPC}
