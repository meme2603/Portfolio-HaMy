  ...
  return (
    ...
    <section className="w-full bg-background text-font py-10">
      <div className="w-full mx-auto">
        ...
        <div className="w-full pt-24 pb-4 px-20 bg-font">
          <div className="flex justify-between items-center mb-12">
            <h3 className="text-[54px] md:text-[64px] font-bold text-primary tracking-tight font-poppins leading-none">
              他にも見る
            </h3>

            {/* 🛠️ BỘ ĐIỀU HƯỚNG: Thêm onClick kích hoạt hàm scrollLeft qua Ref */}
            <div className="flex gap-4 text-background pr-2 select-none">
              <button
                onClick={() => {
                  if (scrollRef.current) scrollRef.current.scrollLeft -= 390; // Trượt sang trái
                }}
                className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-background hover:text-primary active:scale-95 transition-all duration-200"
              >
          ...
          {/* 🛠️ THANH TRƯỢT CAROUSEL: Thay grid bằng flex, gỡ bỏ .slice(0, 3) để lôi hết 5 card ra trượt */}
          <div
            ref={scrollRef}
            className="flex gap-6 overflow-x-auto scroll-smooth mb-24 snap-x snap-mandatory pb-4"
            style={{ scrollbarWidth: "none", msOverflowStyle: "none" }} // Giấu thanh cuộn xấu xí đi bà nha
          >
            {allProjects
              .filter((p) => p.id !== id)
              .map((item) => (
                // Ép kích thước tối thiểu cho mỗi cái Card để nó xếp hàng ngang chuẩn khum bị bóp méo
                <div
                  key={item.id}
          ...
          <div className="w-full border-t border-[#1A120B]/10 pt-6 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs md:text-sm font-light text-[#1A120B]/80 tracking-wide">
            {/* Bên trái: Breadcrumb điều hướng động */}
            <div className="flex items-center gap-2 font-sans">
              <Link to="/" className="hover:underline text-[#1A120B]/60">
                Top
              </Link>
              <span className="text-[#1A120B]/40">&gt;</span>
              <span className="text-[#1A120B]/60">Projects</span>
              <span className="text-[#1A120B]/40">&gt;</span>
            ...
            {/* Bên phải: Bản quyền thương hiệu */}
